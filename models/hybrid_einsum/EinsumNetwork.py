...
class HybridArgs(object):
    """
    Arguments for EinsumNetwork class with HybridLeafLayer as an input layer.
...
class EinsumNetwork(torch.nn.Module):
    ...
    def __init__(self, graph, args=None):
        ...
        elif isinstance(self.args, HybridArgs):
            einet_layers = [HybridLeafLayer(self.graph_layers[0],
                                            self.args.num_dims,
                                            self.args.exponential_family_args)]
        else:
            raise "Unknown args type"
    ...
    def backtrack(self, num_samples=1, class_idxs=None, x=None, mode='sampling', **kwargs):
        ...
        for layer in reversed(self.einet_layers):
            ...
            elif type(layer) == FactorizedLeafLayer or type(layer) == HybridLeafLayer:
